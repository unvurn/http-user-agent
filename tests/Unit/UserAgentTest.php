...
class UserAgentTest extends TestCase
{
    ...
    public function testUserAgentWithUnityPlayerSample()
    {
        ...
        assertEquals("UnityWebRequest/1.0, libcurl/8.5.0-DEV", $userAgent->productVersion("UnityPlayer:comment"));
    }
    ...
    public function testUserAgentWithChromeOnMacSample()
    {
        ...
        assertEquals("Macintosh; Intel Mac OS X 10_15_7", $userAgent->productVersion("Mozilla:comment"));
        assertEquals("537.36", $userAgent->productVersion("AppleWebKit"));
        assertEquals("KHTML, like Gecko", $userAgent->productVersion("AppleWebKit:comment"));
    ...
    public function testUserAgentWithChromeOnMacAndNestedCommentSample()
    {
        ...
        assertNotNull($userAgent);
        assertEquals("5.0", $userAgent->productVersion("Mozilla"));
        assertEquals("Macintosh; Intel Mac OS X 10_15_7 (nested comment is here) and (another one)", $userAgent->productVersion("Mozilla:comment"));
        assertEquals("537.36", $userAgent->productVersion("AppleWebKit"));
        assertEquals("KHTML, (nested comment 2) like Gecko", $userAgent->productVersion("AppleWebKit:comment"));
        assertEquals("126.0.0.0", $userAgent->productVersion("Chrome"));
        assertEquals("537.36", $userAgent->productVersion("Safari"));
    }
}
